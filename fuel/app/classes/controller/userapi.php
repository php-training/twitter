	...
	class Controller_UserAPI extends Controller{
		...
		*/
		...
		public function action_login(){
			...
			$val -> add_field('password','Password','required|min_length[3]|max_length[10]|valid_string[alpha,numeric,dashes,punctuation]');
		...
		//	echo "Hi! I'm signing up an account for you.";
			...
			if (!$val->run()){
				...
				try{
					$num_user=count(Model_User :: find('all', array(
							'where' => array(
									array('email',$email))
					)));
					if ($num_user>0) //validate paramenter, 401
					{
						...
						$new_user             = new Model_User(); 	//insert into databaseateime
